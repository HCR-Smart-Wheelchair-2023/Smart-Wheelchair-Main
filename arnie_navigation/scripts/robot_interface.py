...
class rmv_ros_simple:
    ...
    def __init__(self, pi_ip, pi_port, topic_name="/joy"):
        ...
        self.invert_factor = {"amiga": -1, "yumi": 1}[os.getenv("RNET_ROBOT")]

        #subscriber
        if os.getenv("RNET_JOYSTICK") == "robot":
            self.sub = rospy.Subscriber(topic_name, Joy, self.call_back_joy, queue_size=1)
        elif os.getenv("RNET_JOYSTICK") == "external":
            if topic_name == "/joy": topic_name="/cmd_vel" 
            self.sub = rospy.Subscriber(topic_name, Twist, self.call_back_twist, queue_size=1)
        else:
            rospy.logerr("Command mode not recognised; must be 'robot' or 'external', but got '%s'", os.getenv("RNET_JOYSTICK"))
            raise ArgumentError
