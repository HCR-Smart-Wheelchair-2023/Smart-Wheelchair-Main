...
class MapStitcher:
    def __init__(self):
        ...
        self.sub2 = rospy.Subscriber('/zed/grid_prob_new', OccupancyGrid, self.dynamic_map_callback)
    ...
    def merge_maps(self):
        ...
        x_min = self.dynamic_map.info.center.origin.position.x - int(self.dynamic_map.info.height/2)
        x_max = self.dynamic_map.info.center.origin.position.x - ceil(self.dynamic_map.info.height/2)
        y_min = self.dynamic_map.info.center.origin.position.y - int(self.dynamic_map.info.width/2)
        y_max = self.dynamic_map.info.center.origin.position.y - ceil(self.dynamic_map.info.width/2)
